...
"""Shared GCS retry policy.

Pass ``GCS_RETRY`` as the ``retry=`` kwarg on Cloud Storage operations
(``blob.upload_from_filename``, ``blob.download_to_filename``,
``blob.download_as_*``, ``bucket.list_blobs``, etc.) so transient network
failures back off and retry instead of failing the surrounding pipeline.
"""
