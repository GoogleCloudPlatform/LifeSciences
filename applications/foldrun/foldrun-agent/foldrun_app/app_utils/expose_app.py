                ...
                if "text" in message:
                    ...
                    if isinstance(data, dict):
                        ...
                        if "setup" in data:
                            ...
                            logging.info("Received setup message (not forwarding to agent)")
                            continue

                        # Frontend handles message format for both modes
                        await self.input_queue.put(data)
                    else:
                        logging.warning(f"Received unexpected JSON structure from client: {data}")
                    ...
                    logging.warning(f"Received unexpected message type from client: {message}")
    ...
    async def run_agent_engine(self) -> None:
        ...
        try:
            if self.agent_engine is not None:
                ...
                async for response in self.agent_engine.bidi_stream_query(self.input_queue):
                ...
                assert self.remote_config is not None, "remote_config must be set for remote mode"
                ...
                while True:
                    try:
                        ...
                        if response is not None:
                            ...
                            transformed = self._transform_remote_agent_engine_response(response)
                            if transformed:
                                await self.websocket.send_json(transformed)

                            # Check for error responses
                            if isinstance(response, dict) and "error" in response:
                                logging.error(f"Remote agent engine error: {response['error']}")
                                break
                    except Exception as e:
...
def get_connect_and_run_callable(websocket: WebSocket, config: dict[str, Any]) -> Callable:
    ...
    async def connect_and_run() -> None:
        if config["use_remote_agent"]:
            # Remote agent engine mode
            logging.info(f"Connecting to remote agent engine: {config['remote_agent_engine_id']}")
            remote_config = {
            ...
            logging.info(f"Starting local agent engine with object: {type(agent_engine).__name__}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--project-id", type=str, help="GCP project ID (required when mode=remote)")
    parser.add_argument(
        "--location",
        type=str,
        default="us-central1",
    ...
    if args.mode == "remote":
        ...
        if not args.remote_id:
            ...
            if deployment_metadata_path.exists():
                with open(deployment_metadata_path) as f:
                    ...
                    config["remote_agent_engine_id"] = metadata.get("remote_agent_engine_id")
                    if not config["remote_agent_engine_id"]:
                        parser.error("No remote_agent_engine_id found in deployment_metadata.json")
                    print("Loaded remote agent engine ID from deployment_metadata.json")
            else:
                parser.error("--remote-id is required when deployment_metadata.json is not found")
        else:
        ...
        if not args.project_id:
            ...
            if match:
                ...
                config["location"] = (
                    args.location if args.location != "us-central1" else extracted_location
                )
                ...
                try:
                    ...
                    print(f"Using default project ID from google.auth: {config['project_id']}")
                except Exception as e:
