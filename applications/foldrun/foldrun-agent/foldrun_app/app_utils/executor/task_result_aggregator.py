            ...
            for item in message.content:
                if isinstance(item, dict):
                    ...
                    if content_type == "image":
                        self._media_parts.append(self._convert_media_to_a2a_part(item, "image"))
                    elif content_type == "audio":
                        self._media_parts.append(self._convert_media_to_a2a_part(item, "audio"))
                    elif content_type == "video":
                        self._media_parts.append(self._convert_media_to_a2a_part(item, "video"))

    def _convert_media_to_a2a_part(self, content: dict[str, str], media_type: str) -> Part:
        """Convert a media content block to an A2A Part."""

        mime_type = content.get("mime_type")

        if "url" in content:
            return Part(root=FilePart(file=FileWithUri(uri=content["url"], mime_type=mime_type)))
        elif "base64" in content:
            return Part(
                root=FilePart(file=FileWithBytes(bytes=content["base64"], mime_type=mime_type))
            )
        elif "file_id" in content:
            # For now, store file_id as a URI
            return Part(
                root=FilePart(
                    file=FileWithUri(uri=f"file://{content['file_id']}", mime_type=mime_type)
                )
            )
