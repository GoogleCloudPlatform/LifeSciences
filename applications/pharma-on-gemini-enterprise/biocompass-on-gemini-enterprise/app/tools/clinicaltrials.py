...
def _normalize_study(study: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "completion_date": (status.get("primaryCompletionDateStruct") or {}).get(
            "date"
        ),
...
async def search_clinical_trials(
    ...
    if phase:
        params["filter.advanced"] = " AND ".join(
            f"AREA[Phase]({p.strip()})" for p in phase.split(",")
        )
